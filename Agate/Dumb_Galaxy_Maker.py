#Adapted from the 2015 code cosmic-pop by Liam Keeley 2023 (see https://github.com/COSMIC-PopSynth/COSMIC/blob/develop/bin/cosmic-pop)
#The adaption essentially takes everything smart about COSMIC pop and makes it dumb so that we can generate an entire galaxy with it
#Rather than just fixed populations.
#Rather than looping until statistical convergence as cosmic-pop does, this code simply loops until a user specified amount of mass has been sampled.
#This code also filters out systems outside of LISA's band, which cosmic-pop does not. We will be dealing with large files, and this will slim them down a good bit.
#This still takes cosmic-pop inifile, although the convergence params won't due anything. This is not the cleanest adaption, but... it works. 
import time
import pandas as pd
import numpy as np
import argparse
...
def parse_commandline():
    ...
    parser.add_argument("--total_mass", help="Gives the total mass to sample in creating population", type=float) #Added by Liam Keeley in Galaxy_Maker.py; gives total mass at which to end iterations
...
if __name__ == '__main__':
    ...
    #Note: If we stop iteration, it should restart in the same place if thse files exist.
    try:
    ...
    while total_mass_stars < args.total_mass:
        ...
        rand_seed = seed_int + Nstep #(Nstep updated at end of loop)
        ...
        if sampling['sampling_method'] == 'multidim': #We mostly use this one!
            init_samp_list = InitialBinaryTable.sampler(format_ = sampling['sampling_method'],
                                                        final_kstar1 = kstar1_range,
                                                        final_kstar2 = kstar2_range,
                                                        keep_singles = args.keep_singles,
        ...
        # for so that we know when to stop!
        ...
        #Otherwise, might run into trouble with size of files
        #Also, we will only keep the systems we are interested in–LK
