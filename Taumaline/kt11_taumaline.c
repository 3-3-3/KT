...
int main()
{
    ...
    ifp = fopen("gx_dat_full_taumaline.csv","r");      /* specify input file */
    ...
    int      binnum;
    float    tphys, m1, m2, kstar1, kstar2, lum1, lum2, teff1, teff2, omega_spin_1, omega_spin_2, t_birth, t_evol_GW, sep, ecc, Porbday, xGx, yGx, zGx, dist;
                                                            ...
                                                            /*sep = final seperation (units of solar radius??)*/
                                                            /* rest not used by this program */
    
    float   snrcutoff = 1.0;                                /* cutoff value for SNR */
    int     inputfilelines = 32796297;                      /* number of lines in input data file */
   ...
   /* ------------------ useful fractions ----------------------- */
    ...
    fputs("# BinNum SigType WDbintype Freq(Hz) dhnorm(m) md(solar) ma(solar) mchirp tbirth Porb(s) dist sep SNR\n",ofp1);
    fputs("BinNum SigType WDbintype Freq dhnorm md ma mchirp tbirth Porb dist sep SNR\n",ofp1);     /* this line is for pandas dataframe headers */
    ...
    fputs("# BinNum SigType WDbintype Freq(Hz) dhnorm(m) md(solar) ma(solar) mchirp(solar) tbirth Porb(s) dist sep SNR\n",ofp3);
...
/*  printf("i: %i\n",i); */
    ...
    while (i <= inputfilelines) {                 /* iteration maximum specified above   */
        ...
        fscanf(ifp,"%i,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f",
               &binnum, &tphys, &m1, &m2, &kstar1, &kstar2, &lum1, &lum2, &teff1, &teff2, &omega_spin_1, &omega_spin_2, &t_birth, &t_evol_GW, &sep, &ecc, &Porbday, &xGx, &yGx, &zGx, &dist);                /* read in data point */
    ...
    /* ------------- printing results to files --------------- */
        ...
        if (WDbintype > 0 && WDbintype < 7) {
            if (snr > snrcutoff){             /* only keeping points with SNR above cutoff  */
                ...
                fprintf(ofp1, "%i %i %i %g %g %g %g %g %g %g %g %g %g\n",
                    binnum, sigtype, WDbintype, f, dhnorm, md, ma, mchirp, t_birth, Porb, dist, sep, snr);
                                                    /* write results to file */
            ++detectcnt;                             /* count number of detectable sources */
            } else {
                fprintf(ofp2, "%i %i %i %g %g %g %g %g %g %g %g %g %g\n",
                        binnum, sigtype, WDbintype, f, dhnorm, md, ma, mchirp, t_birth, Porb, dist, sep, snr);
            };
        ++totDWDcnt;                                   /* count total number of sources checked */
         ...
         if (WDbintype > 6 && WDbintype < 16) {
             if (snr > snrcutoff){             /* only keeping points with SNR above cutoff  */
                                                         /* and within LISA band */
                 fprintf(ofp3, "%i %i %i %g %g %g %g %g %g %g %g %g %g\n",
                     binnum, sigtype, WDbintype, f, dhnorm, md, ma, mchirp, t_birth, Porb, dist, sep, snr);
                                                     /* write results to file */
             ++detectcnt;                             /* count number of detectable sources */
             } else {
                 fprintf(ofp4, "%i %i %i %g %g %g %g %g %g %g %g %g %g\n",
                         binnum, sigtype, WDbintype, f, dhnorm, md, ma, mchirp, t_birth, Porb, dist, sep, snr);
             };
